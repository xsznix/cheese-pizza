...
var Code = React.createClass({
	...
	render: function () {
		return (
			React.createElement("pre", {className: "code", ref: "main"}, 
				this.props.data
			));
	}
});
...
var Content = React.createClass({
		...
		if (splits.length === 1)
			...
			return React.DOM.span({},
				node.nodeValue.split('$$').map(function (text, i) {
					if (i % 2) {
						return React.createElement(TeX, {content: text})
					} else {
						return text;
					}
				}));
	...
	prepareNode: function (node, i, arr, isTopLevel) {
		...
		if (tag === 'pre') {
			return React.createElement(Code, {data: node.innerText})
		} else {
	...
	render: function () {
		return (
			React.createElement("div", {className: this.props.className || '', ref: "main"}, 
				this.makeContent()
			));
	}
});